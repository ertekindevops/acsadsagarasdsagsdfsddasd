...
<div class="row">
 ...
 <?php $satilanurunler  =$vt->query("select * from satilanurunler Order By id DESC")->fetchAll(PDO::FETCH_ASSOC); foreach ($satilanurunler as $satilanurunler) {?>
                                                ...
                                                <tr>
													...
													<td><?php echo $satilanurunler["satmail"];?></td>
                                                    <td><?php echo $satilanurunler["fiyat"];?> TL</td>
...
<a style=" font-size:13px;" class="btn btn-danger" href="?id=<?php echo $satilanurunler["id"] ?>&sipsil=ok">
    ...
    function fireSweetizin<?php echo $satilanurunler["id"] ?>() {
        Swal.fire(
            '<img style="height:140px; border-radius:10px;" src="../<?php echo $satilanurunler["resim"] ?>"><br>',
      ...
      if (isset($_POST["guncelle"])) {
		...
		$urunad  	 =$_POST["urunad"];
		$buton  	 =$_POST["buton"];
              $uyeguncel=$vt->prepare("update satilanurunler set fiyat=?, urunad=?, buton=? where id=? ");
              $uyeguncel->execute(array($fiyat,$urunad,$buton,$id));
              if ($uyeguncel) {
